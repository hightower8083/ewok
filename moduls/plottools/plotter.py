...
class plotter:
    ...
    def phase_data_read(self,time):
        ...
        for name in self.names:
            if fnmatch(name, file_mask[self.val_name]+'*_'+time+'.npy'):
                ...
                for name in names_time:
                    ...
                    self.data,self.ax1,self.ax2 = np.histogram2d(data1, data2, \
    ...
    def phase_plotter(self):
        ...
        if plot_domain[0]=='':
            ...
            xmin, xmax,ymin,ymax, vmin,vmax = np.array(plot_domain,dtype='float')
        ...
        if bins_raw[0]=='':
            ...
            self.bins_x, self.bins_y = np.array(bins_raw, dtype ='i')
    ...
    def grid_plotter(self):
        ...
        for time in self.times:
            ...
            if self.val_name == 'chrg':
                self.data = np.load(out_fold['idns']+file_mask['idns']+time+'.npy')-\
                            np.load(out_fold['dns']+file_mask['dns']+time+'.npy')
            else:
                self.data = np.load(out_fold[self.val_name]+file_mask[self.val_name]+time+'.npy')
